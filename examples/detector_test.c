...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *ip, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    list *options = read_data_cfg(datacfg);
    char *name_list = option_find_str(options, "names", "data/names.list");
    char **names = get_labels(name_list);
    ...
    //char *mv;
    //mv=malloc(100);
   // strcpy(mv,"mv predictions.jpg ./img/");
    FILE *log;
    int i = 0;
    bool t = true;
    while(t){
      sprintf(input, "./images/%s_%d.jpg", ip, n);  
      sprintf(temp, "./img/temp_%d.jpg", n);
      
      sprintf(output, "./img/%s_%d.jpg", ip, n); 
//    fprintf(log,"flag: %d\n", access(inputn,0));
      if((access(input,0))!=-1){
  //      log=fopen("time_log.dat","a");
    printf("3\n");
        i = 0;
        ...
        time=what_time_is_it_now();
        network_predict(net, X);
        printf("%s: Predicted in small %f seconds.\n", input, what_time_is_it_now()-time);
//        fprintf(log,"%s: Predicted in small %f seconds.\n", input, what_time_is_it_now()-time);
//        fclose(log);
	...
	if(rename(temp, output)<0){
        ...
        printf("%s: Predicted in large %f seconds.\n", input, what_time_is_it_now()-time);
//		fprintf(log,"%s: Predicted in large %f seconds.\n", input, what_time_is_it_now()-time);
//		fclose(log);
	memset(input,0,200);
        memset(temp,0,200);
        memset(output,0,200);
...
void run_detector(int argc, char **argv)
{
    ...
    if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, "ip", filename, thresh, hier_thresh, outfile, fullscreen);
